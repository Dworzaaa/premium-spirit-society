...
<t:menuLVL1>
    ...
    <jsp:body>
        <h2>
            ...
            <c:choose>
                <c:when test="${isAdmin}">
                    <a href="<c:url value="/user/${user.username}/overview" />">${user.username}</a>
                </c:when>
                <c:otherwise>
        ...
        <form:form id="passwordChangeForm" class="form-horizontal"

                   action="/user/${user.username}/change-password"
                   method="POST"
                   commandName="user">
